...
const App = () => {
  ...
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged(async (firebaseUser) => {
      if (firebaseUser) {
        ...
        if (!userProfile) {
            console.log(`User profile for ${firebaseUser.uid} not found, creating fallback profile...`);
            ...
            const newProfileData: SignupData = { email: email || '', firstName, lastName, password: '', userRole: UserRole.COUREUR };
  ...
  const handleRegister = async (data: SignupData): Promise<{ success: boolean; message: string; }> => {
      try {
          ...
          await firebaseService.createUserProfile(uid, data);
          return { success: true, message: '' };
      } catch (error: any) {
  ...
  const handleCreateTeam = async (teamData: { name: string; level: TeamLevel; country: string; }) => {
    if (!currentUser) return;
    try {
      await firebaseService.createTeamForUser(currentUser, teamData);
      // After creation, reload user data to get the new active team
      const updatedUser = await firebaseService.getUserProfile(currentUser.id);
      if(updatedUser) {
  ...
  const navigateTo = (section: AppSection, eventId?: string) => {
    if (section === 'eventDetail' && eventId) {
      ...
      // Do not change currentSection here to allow going "back" to the previous section
    } else {
  ...
  const handleUserManagementAction = async (action: () => Promise<void>) => {
    try {
        await action();
        if (currentUser) {
            await loadDataForUser(currentUser); // Reload all data to reflect changes
        }
    } catch (error) {
        console.error("User management action failed:", error);
        alert("Une erreur est survenue lors de la gestion de l'utilisateur.");
    }
  };

  const handleApproveMembership = (membership: TeamMembership) => {
    if (!membership.id) {
        console.error("Membership ID is missing, cannot approve.");
        return;
    }
    handleUserManagementAction(() => 
        firebaseService.approveMembership(membership.id!, membership.teamId, membership.userId, membership.userRole)
    );
  };
  
  const handleDenyMembership = (membership: TeamMembership) => {
    if (!membership.id) {
        console.error("Membership ID is missing, cannot deny.");
        return;
    }
    handleUserManagementAction(() =>
        firebaseService.denyOrRemoveMembership(membership.id!, membership.userId, membership.teamId, false)
    );
  };

  const handleRemoveUser = (userId: string, teamId: string) => {
    const membership = appState.teamMemberships.find(m => m.userId === userId && m.teamId === teamId);
    if (!membership?.id) {
        alert("Impossible de trouver l'adhésion pour cet utilisateur.");
        return;
    }
    handleUserManagementAction(() => 
        firebaseService.denyOrRemoveMembership(membership!.id!, userId, teamId, true)
    );
  };
  
  const handleUpdateUserTeamRole = (userId: string, teamId: string, newUserRole: UserRole) => {
      const membership = appState.teamMemberships.find(m => m.userId === userId && m.teamId === teamId);
      if (!membership?.id) {
        alert("Impossible de trouver l'adhésion pour cet utilisateur.");
        return;
      ...
      handleUserManagementAction(() =>
          firebaseService.updateUserTeamRole(membership!.id!, newUserRole)
      );
  };
  
  const handleUpdateUserPermissionRole = (userId: string, newPermissionRole: TeamRole) => {
      handleUserManagementAction(() => 
          firebaseService.updateUserPermissionRole(userId, newPermissionRole)
      );
  };
  
  const handleUpdateUserCustomPermissions = (userId: string, newPermissions: Partial<Record<AppSection, PermissionLevel[]>>) => {
      handleUserManagementAction(() =>
          firebaseService.updateUserCustomPermissions(userId, newPermissions)
      );
  };
            ...
            <main className="flex-grow ml-64 p-6 bg-gray-100 min-h-screen">
                {activeEvent ? (
                    ...
                    <>
                        ...
                        {currentSection === 'userManagement' && <UserManagementSection appState={appState} currentTeamId={appState.activeTeamId!} onApprove={handleApproveMembership} onDeny={handleDenyMembership} onInvite={()=>{}} onRemove={handleRemoveUser} onUpdateRole={handleUpdateUserTeamRole} onUpdatePermissionRole={handleUpdateUserPermissionRole} onUpdateUserCustomPermissions={handleUpdateUserCustomPermissions} onTransferUser={()=>{}}/>}
